...
Trip-time sensitivity (4 POLLEN/trip, one robot): 8 s → 7.4 TIPs; 12 s → 5.3; 18 s → 3.6;
30 s → 2.7. **Going from a 15 s cycle to a 10 s cycle is worth more than any mechanism
upgrade in this document.** And TIPs are *alliance* points — both robots shoot the same
...
|---|---|---|---|
| 1 | **Cycle time > 15 s** | 5.3 TIPs → 3.6 TIPs; POLLINATOR 2 RP lost | Design for a 10 s trip: short travel, big mouth, no waiting on the flywheel. Practise the 4-ball cycle as a drill. |
